...
export const NODE_DEFS = {
  grayscale: {
    label: 'Grayscale',
    description: 'Collapse colour to a single brightness value. Usually the first step so later operators work on intensity rather than colour.',
  ...
  threshold: {
    ...
    description: 'Convert to pure black/white. Adaptive mode compares each pixel to its local neighbourhood mean to survive uneven lighting; absolute mode uses one global cutoff.',
  ...
  gaussian: {
    label: 'Gaussian Blur',
    description: 'Smooth the image with a separable Gaussian blur to reduce noise and speckle, often before thresholding.',
  ...
  upscale: {
    label: 'Upscale',
    description: 'Enlarge the image. Nearest-neighbour keeps hard ink edges crisp; bilinear interpolates for smoother gradients.',
  ...
  erosion: {
    label: 'Erosion',
    description: 'Shrink black ink: removes isolated speckle and thins strokes (treats dark pixels as foreground).',
  ...
  dilation: {
    label: 'Dilation',
    description: 'Grow black ink: thickens faint strokes and closes small gaps (treats dark pixels as foreground).',
    defaultConfig: () => ({ kernelSize: 3 }),
    fields: [
      { key: 'kernelSize', label: 'Kernel size (odd)', type: 'number', min: 1, max: 25, step: 2 },
    ],
  ...
  convolution: {
    label: 'Convolution',
    description: 'Apply a custom weighted kernel for sharpen, edge-detect, line-bridging, emboss and other effects.',
  ...
  mask: {
    ...
    description: 'Restrict the following nodes to — or protect — rectangular regions you draw on the input image.',
