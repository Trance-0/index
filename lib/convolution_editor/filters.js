...
// ---------------------------------------------------------------------------
//
// Templates come in two flavours so loading one can respect whatever kernel
// size the user has already chosen (we never silently resize their kernel):
//
//   * GENERIC – has a generation algorithm, so it can be built at ANY width ×
//     height (identity, box blur, line bridges). Loading regenerates the
//     weights at the current size.
//   * FIXED 3×3 – a specific 3×3 stencil with no natural larger form (sharpen,
//     edge detect, Laplacian, emboss). Loading CENTERS the 3×3 pattern inside
//     the current kernel (zeros around it). These require the kernel to be at
//     least 3×3; the editor labels them "(3×3)" and disables them otherwise.

// --- generic generators (return a flat weights array of length w*h) ---
function genCenterOne(w, h) {
  const a = new Array(w * h).fill(0);
  a[Math.floor(h / 2) * w + Math.floor(w / 2)] = 1;
  return a;
}
function genAllOnes(w, h) {
  return new Array(w * h).fill(1);
}
function genMiddleRow(w, h) {
  const a = new Array(w * h).fill(0);
  const my = Math.floor(h / 2);
  for (let x = 0; x < w; x++) a[my * w + x] = 1;
  return a;
}
function genMiddleCol(w, h) {
  const a = new Array(w * h).fill(0);
  const mx = Math.floor(w / 2);
  for (let y = 0; y < h; y++) a[y * w + mx] = 1;
  return a;
}
// Place a 3×3 `base` stencil centered inside a w×h grid (zeros elsewhere).
function centerBase3(base, w, h) {
  const a = new Array(w * h).fill(0);
  const ox = Math.floor((w - 3) / 2);
  const oy = Math.floor((h - 3) / 2);
  for (let by = 0; by < 3; by++) {
    for (let bx = 0; bx < 3; bx++) {
      a[(oy + by) * w + (ox + bx)] = base[by * 3 + bx];
    }
  }
  return a;
}

// Internal definitions. `flags` are the normalize/clamp/abs that suit each
// template; `generic` templates carry a `gen(w,h)`, fixed ones carry `base`.
const TEMPLATE_DEFS = {
  identity: {
    label: 'Identity', generic: true,
    flags: { normalize: false, clamp: true, abs: false },
    gen: genCenterOne,
  },
  box_blur: {
    label: 'Box blur', generic: true,
    flags: { normalize: true, clamp: true, abs: false },
    gen: genAllOnes,
  },
  horizontal_bridge: {
    label: 'Horizontal line bridge', generic: true,
    // Middle row of 1s emphasizes horizontal continuity (reconnects strokes).
    flags: { normalize: true, clamp: true, abs: false },
    gen: genMiddleRow,
  },
  vertical_bridge: {
    label: 'Vertical line bridge', generic: true,
    flags: { normalize: true, clamp: true, abs: false },
    gen: genMiddleCol,
  },
  sharpen: {
    label: 'Sharpen', generic: false,
    flags: { normalize: false, clamp: true, abs: false },
    base: [0, -1, 0, -1, 5, -1, 0, -1, 0],
  },
  edge_detect: {
    label: 'Edge detect', generic: false,
    flags: { normalize: false, clamp: true, abs: true },
    base: [-1, -1, -1, -1, 8, -1, -1, -1, -1],
  },
  laplacian: {
    label: 'Laplacian', generic: false,
    flags: { normalize: false, clamp: true, abs: true },
    base: [0, 1, 0, 1, -4, 1, 0, 1, 0],
  },
  emboss: {
    label: 'Emboss', generic: false,
    flags: { normalize: false, clamp: true, abs: false },
    base: [-2, -1, 0, -1, 1, 1, 0, 1, 2],
  },
};

// Can this template be applied at the given kernel size?
// Generic templates fit any size; fixed 3×3 stencils need width,height >= 3.
export function templateFits(key, width, height) {
  const t = TEMPLATE_DEFS[key];
  if (!t) return false;
  return t.generic || (width >= 3 && height >= 3);
}

// Build a template at a specific kernel size. Returns the weights (sized to
// width*height) plus the template's normalize/clamp/abs flags. NOTE: it does
// NOT include width/height — callers keep the user's current size. Returns null
// when the template does not fit (e.g. a fixed 3×3 into a 2×2 kernel).
export function buildKernelTemplate(key, width = 3, height = 3) {
  const t = TEMPLATE_DEFS[key];
  if (!t) return null;
  if (t.generic) {
    return { weights: t.gen(width, height), ...t.flags, generic: true };
  }
  if (width >= 3 && height >= 3) {
    return { weights: centerBase3(t.base, width, height), ...t.flags, generic: false };
  }
  return null;
}

// Public template list for the editor UI. `make()` produces a full 3×3 config
// (used for node defaults / presets); `generic` drives the size labelling.
export const KERNEL_TEMPLATES = Object.fromEntries(
  Object.entries(TEMPLATE_DEFS).map(([key, t]) => [key, {
    label: t.label,
    generic: t.generic,
    make: () => {
      const b = buildKernelTemplate(key, 3, 3);
      return { width: 3, height: 3, weights: b.weights, normalize: b.normalize, clamp: b.clamp, abs: b.abs };
    },
  }]),
);
